...
/**
 * /dev/contact — desktop port of canonical /contact against desktop-spec.md.
 *
 * Mirrors the same upgrade pattern applied to /dev/about, /dev/faq, and
 * /dev/faq-download in earlier sessions. Mobile JSX matches canonical
 * /contact verbatim. Differences from canonical, all desktop-only:
 *
 *   1. `desktop-experiment dev-contact` outer marker so the
 *      `.desktop-experiment` CSS scope kicks in at ≥768px.
 *   2. Side-nav `<aside class="dev-side-nav">` block — replaces
 *      FloatingNav at ≥1200px. /contact filtered out (current route),
 *      mirroring how /about and /faq filter their own routes.
 *   3. H1 closing period in purple — cross-page brand tic per
 *      desktop-spec.md ("every page H1 ends with `text-purple` period").
 *      Canonical /contact was the holdout missing this.
 *   4. Email IDE SectionTile gains `tile-right-edge` className for
 *      right-gutter overhang, matching the treatment /about applied to
 *      its two IDE CTAs.
 *
 * No copy rewrites, no rhythm reordering, no hard-breaks. All further
 * composition decisions await Alex's review.
 */
